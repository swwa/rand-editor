...
Flag    puflg;
{
    ...
    legetline (line);     /* must do this to set ncline */
    if (col >= ncline - 1)
	return CROK;
    ...
    legetline (line);         /* yes, we must do it again */
    Block {
	Reg4 Ncols  nsave;
	Reg5 Char   csave;
...
Flag puflg;
{
	...
	for (itmp = 0; itmp < nlines; itmp++) Block {
	    ...
	    legetline (line + itmp);
	    if ((j = (ncline - col)) > 0) {
		legetline (-1);    /* because we are about to corrupt cline */
		j = min (j, ncols);
		(cp = &cline[col])[j] = '\n';
 ...
 err1:          errval = lacmderr ("edrect");
	...
	for (itmp = 0; itmp < nlines; itmp++) {
	    ...
	    legetline (line + itmp);
	    legetline (-1);  /* because we are about to do a dechars on cline */
