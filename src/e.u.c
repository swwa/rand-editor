...
Flag    puflg;
{
    ...
    char    *origfile;
#endif SYMLINKS
    static  noaskonrecover = YES;

    if (file == (char *)NULL || *file == 0)     /* If null filename */
...
/* 3/7/84 changed to handle null directory */
	...
	if ((j = filetype (file)) != -1) {
	    ...
	    nopens++;
            {
		char xyz[20]; 
		sprintf (xyz,", length is %ld.", (long)la_lsize(nlas));
		mesg(TELSTRT+3, "EDIT: ", file, xyz);
	    }
	    d_put (0);
...
#endif SYMLINKS
    if (nameexpanded && file != (char *)NULL)
	sfree (file);
    if (dir != (char *)NULL)
	sfree (dir);
    return retval;
...
char *file;
{
    ...
    if (*linkname == '/' || ((cp = rindex (file, '/')) == (char *)NULL))
...
char *file;
{
	...
	for (beg = tmp, xend = file+1; *xend && *xend != '/'; )
	    *beg++ = *xend++;
	*beg = '\0';
	if ((pw = getpwnam(tmp)) == (struct passwd *)NULL) {
	    mesg (ERRALL+2, "No such user: ", tmp);
	    return (char *)NULL;
	}
	strcpy (tmp, pw->pw_dir);
	if (*xend)
	    strcat (tmp, xend);   /* append rest of the name */
    }
