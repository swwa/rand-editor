...
.
    ...
    RUNSAFE must be defined in e.h -- not here

    If safe, then
	e | prog >> chgfile
    else
	e | prog | /usr/local/etc/run >> chgfile
...
Flag    safe;       /* OK for program to write directly on changes file */
{
    static char *runargs[2] = { "/usr/local/etc/run" , "" };
    int  pipe1[2];
    int  child1;
#ifndef RUNSAFE
    int  child2;
...
#endif
	&& xreceive (chgend, from, closeflg ? number : 0, iqbuf, puflg)
       )
	return CROK;
    execabortmesg (funcnm);
    d_put (0);
...
/*
Flag
xreceive (chgend, from, nclose, iqbuf, puflg)
    long    chgend;
    Nlines  from;
    Nlines  nclose;
    Small   iqbuf;
