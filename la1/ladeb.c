...
char *txt;
{
    ...
    printf ("  lpos= %d, bpos= %d, back= 0x%x, forw= 0x%x, sflags= %d\n",
	    ...
	    0,
#endif
	    plas->la_sback, plas->la_sforw, (int)plas->la_sflags);
    return;
}
...
char *txt;
{
    ...
    printf ("nlines= %ld", (long)plaf->la_nlines);
#ifdef LA_BP
    printf (", nbytes= %dl\n", (long)plaf->la_nbytes);
#else
    printf ("\n");
...
char *txt;
{
    ...
    printf ("La_stream chain dump:             %s", txt);
    putchar ('\n');
...
char *txt;
{
    ...
    printf ("La_stream file chain dump:        %s", txt);
    putchar ('\n');
