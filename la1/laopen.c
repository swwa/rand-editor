...
int chan;
{
	...
	if (!filename || *filename == '\0') {
		...
		La_stream *nlas;
     openit:    if ((nlas = la_ffopen (ffl, plas, offset)) == (La_stream *)NULL) {
		    (void) ff_close (ffl);
		    return (La_stream *)NULL;
		}
		return nlas;
...
long pos;
{
    ...
    if ((tlaf = (La_file *) malloc ((unsigned int) sizeof *tlaf)) == (La_file *)NULL) {
	la_errno = LA_NOMEM;
	    goto bad;
    }
...
char          *buf;
{
    ...
    La_linepos      lcnt;       /* fsd line count */
    La_linepos      totlcnt;    /* total # lines in this new chain */
	...
	*buf = 0;
	...
	/* fprintf(stderr,"laparse\n"); */
	if (   lcnt >= LA_FSDLMAX || totchcnt >= nchars
	    || nfsb >= LA_FSDBMAX - 2
	   ) {
 ...
 *                  if ((nlft = ff_point (pffs, curpos, &newcp, (int) totlft))
 */
