/*
/* This file contains all data and routines which are required as an
/* absolute minimum to use the FF Package.
/* One exception: div.c is required, but it should be customized for the
/* CPU used.
/**/
...
int arena;
{
    ...
    while (nbuf--) {
	Reg1 Ff_buf *fb;
	if ((fb = (Ff_buf *) malloc (sizeof (Ff_buf))) == (char *)NULL)
	    break;
	if ((fb->fb_buf = malloc ((unsigned int) FF_BSIZE)) == (char *)NULL) {
	    free ((char *) fb);
	    break;
	}
...
Reg4 int rdflg;
{
 ...
 ismem:
    ...
    /* If the block is not already the first or second block in the free list,
    /* move it to head of free list to mark it as a recent reference
    /**/
...
Reg3 Ff_buf *fb;
int release;    /* if non-0, then release the block from the active chain */
{
    Reg2 Ff_file *fp;
    Reg4 int try;
    Reg5 int cnt;
